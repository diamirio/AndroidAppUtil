...
class EqualRule(private val other: () -> String?, @StringRes override val errorMessage: Int) : Rule<String> {
    override fun validate(input: String?) = input == other.invoke()
}

open class RegexRule(private val pattern: String, @StringRes override val errorMessage: Int) : Rule<String> {
    override fun validate(input: String?) = input != null && pattern.toRegex().matches(input)
}

class MinLengthRule(private val length: Int, @StringRes override val errorMessage: Int) : Rule<String> {
    override fun validate(input: String?): Boolean = input != null && input.length >= length
}

class MaxLengthRule(private val length: Int, @StringRes override val errorMessage: Int) : Rule<String> {
    override fun validate(input: String?): Boolean = input != null && input.length <= length
}
